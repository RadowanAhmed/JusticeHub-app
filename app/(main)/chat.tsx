...
قدم استشارات قانونية دقيقة وواضحة باللغة العربية الفصحى.
...
إذا لم تكن متأكداً من معلومة، قل ذلك بوضوح.`,
          ...
          if (!response.ok) {
            ...
            console.error(
              `OpenRouter API Error Response for ${model}:`,
              errorText,
            );
            lastError = errorText;
          ...
          if (aiResponse.trim().length < 30) {
            continue; // Response too short, try next model
          }
      ...
      // If all models fail, use local response
      console.log("All OpenRouter models failed, using local response");
