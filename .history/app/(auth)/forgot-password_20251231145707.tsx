...
export default function ForgotPasswordScreen() {
  ...
  useEffect(() => {
    const handleAppStateChange = async (nextAppState: string) => {
      if (nextAppState === 'background' || nextAppState === 'inactive') {
        ...
        if ((hasValidSession === 'true' || isResetInProgress) && step !== 'password') {
          console.log('App going to background with incomplete reset - cleaning up');
          setTimeout(async () => {
  ...
  try {
    ...
    // Get your Supabase URL from environment
    const supabaseUrl = process.env.EXPO_PUBLIC_SUPABASE_URL;
    const functionName = 'check-email-exists';
  ...
  try {
    ...
    showSuccess(t('auth.codeSentMessage'));
  ...
  const handleResetSuccess = async () => {
    console.log('Password reset completed successfully - signing out recovery session');
        ...
        >
          {loading ? (
            ...
            <Text style={[styles.buttonText, { 
              fontSize: 16,
              fontWeight: '600',
            }]}>{t('auth.sendVerificationCode')}</Text>
          )}
        </TouchableOpacity>
        ...
        <View style={[styles.codeInputsContainer, { marginBottom: 32 }]}>
          {code.map((digit, index) => (
            <TextInput
              ...
              style={[
                styles.codeInput,
                digit && [styles.codeInputFilled, { borderColor: colors.primary }],
                { 
                  backgroundColor: colors.card, 
        ...
        <View style={[styles.resendContainer, { marginTop: 24 }]}>
          ...
          >
            <Text style={[
              ...
              {countdown > 0 ? `${t('auth.resendIn')} ${formatCountdown(countdown)}` : t('auth.resendCode')}
            </Text>
          </TouchableOpacity>
        </View>
      ...
      <View style={[styles.form, { marginTop: 24 }]}>
        ...
        <View style={[styles.inputContainer, { marginTop: 16 }]}>
          <Text style={[styles.label, { 
            ...
            marginBottom: 8,
            fontWeight: '600',
          }]}>{t('auth.confirmNewPassword')}</Text>
          <View style={[styles.inputWrapper, { 
            backgroundColor: colors.card, 
            borderColor: passwordError ? colors.danger : colors.border,
            borderWidth: 1.5,
        ...
        >
          {loading ? (
            ...
            <Text style={[styles.buttonText, { 
              fontSize: 16,
              fontWeight: '600',
            }]}>{t('auth.resetPasswordButton')}</Text>
          )}
        </TouchableOpacity>
