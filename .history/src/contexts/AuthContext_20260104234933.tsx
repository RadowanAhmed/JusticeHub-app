...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  try {
    ...
    if (data.user && data.session) {
      ...
      try {
        ...
        await pushNotificationService.sendLoginNotification(
          data.user.id,
  ...
  try {
    ...
    // Step 2: Send WELCOME PUSH notification
    try {
      console.log('🔔 Sending welcome push notification...');
      await pushNotificationService.sendSignupNotification(
        authData.user.id,
        cleanEmail,
        cleanFirstName
      );
    ...
    if (signInError) {
      ...
      // Send login push notification even if auto login fails
      try {
        console.log('🔔 Sending login push notification (auto login failed)...');
        await pushNotificationService.sendLoginNotification(
          authData.user.id,
          cleanEmail
        );
    ...
    if (signInData.user && signInData.session) {
      ...
      try {
        ...
        if (pushToken) {
          console.log('💾 Saving push token...');
          await fcmService.savePushTokenToDatabase(signInData.user.id, pushToken);
        }
      } catch (pushError) {
        console.error('❌ Push notification registration failed:', pushError);
      }
      ...
      try {
        ...
        await pushNotificationService.sendLoginNotification(
          signInData.user.id,
          signInData.user.email || cleanEmail
        );
      } catch (notifError) {
        console.error('❌ Login notification failed:', notifError);
      }
