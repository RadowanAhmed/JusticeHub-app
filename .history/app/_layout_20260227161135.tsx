...
export default function RootLayout() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // SAFE CLEANUP: Check if methods exist before calling
      try {
        // Method 1: If subscriptions have a remove method (most common in newer versions)
        if (
          notificationListener.current &&
          typeof notificationListener.current.remove === "function"
        ) {
          notificationListener.current.remove();
          console.log("✅ Removed notification listener using .remove()");
        }
        ...
        if (
          ...
          console.log("✅ Removed response listener using .remove()");
        }
      } catch (error) {
        console.log("⚠️ Error removing with .remove():", error);
      }

      try {
        // Method 2: Try the removeNotificationSubscription method if it exists
        if (
          ...
          if (notificationListener.current) {
            ...
            console.log(
              "✅ Removed notification listener using removeNotificationSubscription",
            );
          }
          if (responseListener.current) {
            ...
            console.log(
              "✅ Removed response listener using removeNotificationSubscription",
            );
          }
        }
      } catch (error) {
        console.log(
          "⚠️ Error removing with removeNotificationSubscription:",
          error,
        );
      }

      // Method 3: Some versions use removeEventListener
      try {
        if (typeof Notifications.removeEventListener === "function") {
          if (notificationListener.current) {
            Notifications.removeEventListener(notificationListener.current);
          }
          if (responseListener.current) {
            Notifications.removeEventListener(responseListener.current);
          }
        }
      } catch (error) {
        console.log("⚠️ Error removing with removeEventListener:", error);
      }
    };
  }, []);
