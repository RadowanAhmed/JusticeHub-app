...
class PasswordResetManager {
  ...
  async isRecoverySessionActive(): Promise<boolean> {
    try {
      ...
      const recoverySessionActive = await AsyncStorage.getItem('recovery_session_active');
      const hasValidResetSession = await AsyncStorage.getItem('has_valid_reset_session');
      
      return recoverySessionActive === 'true' || hasValidResetSession === 'true';
    } catch (error) {
